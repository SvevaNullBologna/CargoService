	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		...
		return { //this:ActionBasciFsm
				...
				state("waitingupdate") { //this:State
					action { //it:State
						CommUtils.outmagenta("$name waiting update from cargoservice")
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
					sysaction { //it:State
					}	 	 
					 transition(edgeName="t00",targetState="filterupdate",cond=whenDispatch("update"))
				}	 
				state("filterupdate") { //this:State
					action { //it:State
						...
						if( checkMsgContent( Term.createTerm("update(HoldJsonString)"), Term.createTerm("update(HoldJsonString)"), 
								...
								emit("filteredupdate", "filteredupdate($FilteredJsonMsg)" ) 
						}
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
